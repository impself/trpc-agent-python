...
class TestExtractCodeBlocks:
    ...
    def test_mapping_with_code_blocks_list(self):
        out = _extract_code_blocks({"code_blocks": [
...
class TestSafetyWrappedCallable:
    def test_constructor_requires_one_script_param(self):
        with pytest.raises(ValueError):
            SafetyWrappedCallable(_guard(), lambda x: x,
                                  tool_name="t")  # neither
        with pytest.raises(ValueError):
            SafetyWrappedCallable(_guard(), lambda x: x,
                                  tool_name="t",
    ...
    def test_safe_call_runs_delegate(self):
        ...
        wrapped = SafetyWrappedCallable(
            _guard(), delegate, tool_name="python_exec",
            language=ScriptLanguage.PYTHON, script_kw="code")
    ...
    def test_script_positional(self):
        ...
        wrapped = SafetyWrappedCallable(
            _guard(), delegate, tool_name="python_exec",
            language=ScriptLanguage.PYTHON, script_pos=0)
    ...
    def test_extract_script_list_value(self):
        ...
        wrapped = SafetyWrappedCallable(
            _guard(), delegate, tool_name="bash_exec",
            language=ScriptLanguage.BASH, script_kw="command")
    ...
    def test_extract_script_out_of_range_position(self):
        ...
        wrapped = SafetyWrappedCallable(
            _guard(), delegate, tool_name="python_exec",
            language=ScriptLanguage.PYTHON, script_pos=5)
    ...
    def test_call_async_blocks_dangerous(self):
        ...
        import asyncio
        with pytest.raises(BlockedExecutionError):
            asyncio.run(wrapped.call_async(
                code="import shutil\nshutil.rmtree('/x')"))
    ...
    def test_cwd_env_argv_propagation(self):
        ...
        def delegate(command, cwd=None, env=None, timeout=None,
                     argv=None, metadata=None, output_bytes=None):
            captured.update(cwd=cwd, env=env, argv=argv,
                            timeout=timeout)
            return "ok"
        ...
        wrapped = SafetyWrappedCallable(
            _guard(), delegate, tool_name="bash_exec",
            language=ScriptLanguage.BASH, script_kw="command",
            cwd_kw="cwd", env_kw="env", timeout_kw="timeout",
            argv_kw="argv", metadata_kw="metadata",
            output_bytes_kw="output_bytes")
        wrapped(command="ls", cwd="/tmp",
                env={"X": "1"}, timeout=30, argv=["ls"])
...
class TestSafetyCheckedExecutor:
    ...
    async def test_output_truncation(self):
        ...
        executor = SafetyCheckedExecutor(
            ToolSafetyGuard(policy), Delegate())
        result = await executor.execute_code({"code": "print('hi')"})
    ...
    async def test_multiple_blocks_combined(self):
        class Delegate:
            async def execute_code(self, inp):
                return {"outcome": "OK", "output": "x"}
        executor = SafetyCheckedExecutor(_guard(), Delegate())
        result = await executor.execute_code({"code_blocks": [
...
class TestRenderExecutorBlock:
    def test_renders_payload(self):
        from trpc_agent_sdk.tools.safety._models import (
            Evidence, RiskCategory, RiskLevel, SafetyDecision,
            SafetyFinding, SafetyReport,
        )
        report = SafetyReport(
            report_id="r",
